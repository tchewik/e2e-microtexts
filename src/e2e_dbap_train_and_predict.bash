# Using: $ bash e2e_dbap_train_and_predict.bash --lang {en|ru} --train_data {en|ru|en_aug|ru_aug}
...
crossval ()
{
    ...
    # Prepare directory
    DIRECTORY=e2e_dbap_${TRAIN_DATA}

    # Prepare configs for K-fold validation
    CONFIGFILE=configs/e2e_dbap_${LANG}_fold_0.jsonnet
    rm configs/e2e_dbap_${LANG}_fold*
    cp initial_configs/e2e_dbap_${LANG}_fold_0.jsonnet $CONFIGFILE
    ...
    do
        TRAIN_DATA=${TRAIN_DATA} allennlp train -s ${DIRECTORY}/fold_${FOLD} configs/e2e_dbap_${LANG}_fold_${FOLD}.jsonnet
...
crossval_tune ()
{
    ...
    do
       ...
       for (( FOLD=0; FOLD<$K; FOLD++ ))
       do
           TRAIN_DATA=${TRAIN_DATA} DROPOUT=${DROPOUT} allennlp train -s ${DIRECTORY}/fold_${FOLD} configs/e2e_dbap_${LANG}_fold_${FOLD}.jsonnet
           allennlp predict --use-dataset-reader --silent \
                            --output-file ${DIRECTORY}/fold_${FOLD}/predictions_test.json ${DIRECTORY}/fold_${FOLD}/model.tar.gz \
                            data/conll_cv_rst_end2end/${LANG}/test_fold_${FOLD}.conll \
                            --include-package dependency_parser
