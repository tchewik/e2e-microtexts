...
from allennlp.modules import Seq2VecEncoder, TextFieldEmbedder, InputVariationalDropout, TimeDistributed
...
from allennlp.nn.util import get_text_field_mask, get_range_vector, sequence_cross_entropy_with_logits
...
@Model.register("span_biaffine_parser")
class SpanBiaffineDependencyParser(Model):
    """
    ...
    def __init__(
        ...
        # role_predictor = role_predictor or FeedForward(
        #     encoder_dim, 2, [100, self.num_roles], [torch.nn.ReLU(), torch.nn.Softmax()], 0.1)
        # self.role_projection_layer = TimeDistributed(role_predictor)
    ...
    def forward(
        ...
        roles_logits = self.role_projection_layer(text_encoded)
        predicted_roles = F.softmax(roles_logits, dim=-1)

        if roles_tags is not None:
            roles_loss = sequence_cross_entropy_with_logits(roles_logits, roles_tags, text_mask,
                                                            alpha=self._roles_alpha)
            loss += roles_loss * self.roles_theta
            self._roles_f1_pro(roles_logits, roles_tags)
            self._roles_f1_opp(roles_logits, roles_tags)
            self._roles_accuracy(roles_logits, roles_tags)

        if head_indices is not None and functions_labels is not None:
    ...
    def make_output_human_readable(
        ...
        for instance_roles, instance_heads, instance_functions, length in zip(roles, heads, functions, lengths):
            role_labels.append(
                [self.vocab.get_token_from_index(role, "roles_tags") for role in instance_roles.argmax(axis=1)])
