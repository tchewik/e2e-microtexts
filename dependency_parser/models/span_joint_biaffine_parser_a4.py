...
@Model.register("span_joint_biaffine_parser_a4")
class SpanJointBiaffineDependencyRestrParserA4(Model):
    """
    ...
    Roles are predicted jointly with the arc tags.
    # Parameters
    ...
    def __init__(
            ...
            role_representation_dim: int,
            role_predictor: FeedForward = None,
            tag_feedforward: FeedForward = None,
            arc_feedforward: FeedForward = None,
            role_feedforward: FeedForward = None,
            use_mst_decoding_for_validation: bool = True,
            input_layernorm: bool = False,
            dropout: float = 0.0,
            input_dropout: float = 0.0,
            roles_theta: float = 0.5,
        ...
        self.arc_attention = BilinearMatrixAttention(
            arc_representation_dim, arc_representation_dim, use_input_biases=True, activation=torch.nn.ReLU()
        )
        ...
        # For ROLE prediction #####
        self.num_roles = self.vocab.get_vocab_size("roles_tags")

        self.head_role_feedforward = role_feedforward or FeedForward(
            encoder_dim, 1, role_representation_dim, torch.nn.ReLU()  # Activation.by_name("elu")()
        )
        self.child_role_feedforward = copy.deepcopy(self.head_role_feedforward)

        self.role_bilinear = torch.nn.modules.Bilinear(
            role_representation_dim, role_representation_dim, self.num_roles
        )
        ...
        check_dimensions_match(
            role_representation_dim,
            self.head_role_feedforward.get_output_dim(),
            "role representation dim",
            "role feedforward output dim",
        )
    ...
    def forward(
        ...
        predicted_heads, predicted_functions_labels, predicted_roles, augmented_mask, arc_nll, tag_nll, role_nll = self._parse(
            text_encoded, text_mask, functions_labels, head_indices, roles_tags)

        loss = arc_nll + tag_nll + role_nll
    ...
    def _parse(
            ...
            head_indices: torch.LongTensor = None,
            role_tags: torch.LongTensor = None,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:

        batch_size, _, encoding_dim = encoded_text.size()

        head_sentinel = self._head_sentinel.expand(batch_size, 1, encoding_dim)
        # print('true head_sentinel shape:', head_sentinel.shape)
        # Concatenate the head sentinel onto the sentence representation.
        # encoded_text = torch.cat([head_sentinel, encoded_text], 1)
        head_sentinel = encoded_text.mean(axis=1).unsqueeze(1)  # (A4) Here we'll use averaged text representation for ROOT instead of random values
        # print('your head_sentinel shape:', head_sentinel.shape)
        encoded_text = torch.cat([head_sentinel, encoded_text], 1)
        ...
        if role_tags is not None:
            role_tags = torch.cat([role_tags.new_zeros(batch_size, 1), role_tags], 1)
        ...
        # shape (batch_size, sequence_length, tag_representation_dim)
        head_role_representation = self._dropout(self.head_role_feedforward(encoded_text))
        child_role_representation = self._dropout(self.child_role_feedforward(encoded_text))
        ...
        if self.training or not self.use_mst_decoding_for_validation:
            predicted_heads, predicted_head_tags, predicted_roles = self._greedy_decode(
                head_tag_representation, child_tag_representation, head_role_representation, child_role_representation,
                attended_arcs, mask
            )
        else:
            predicted_heads, predicted_head_tags, predicted_roles = self._mst_decode(
                head_tag_representation, child_tag_representation, head_role_representation, child_role_representation,
                attended_arcs, mask
            )

        if head_indices is not None and head_tags is not None and role_tags is not None:
            arc_nll, tag_nll, role_nll = self._construct_loss(
                head_tag_representation=head_tag_representation,
                child_tag_representation=child_tag_representation,
                head_role_representation=head_role_representation,
                child_role_representation=child_role_representation,
            ...
            arc_nll, tag_nll, role_nll = self._construct_loss(
                ...
                role_tags=predicted_roles.long(),
                mask=mask,
            )

        return predicted_heads, predicted_head_tags, predicted_roles, mask, arc_nll, tag_nll, role_nll
    ...
    def _construct_loss(
        ...
        # shape (batch_size, sequence_length, num_head_tags)
        head_role_logits = self._get_head_roles(
            head_role_representation, child_role_representation, head_indices
        )
        normalised_head_role_logits = masked_log_softmax(
            head_role_logits, mask.unsqueeze(-1)
        ) * mask.unsqueeze(-1)
        
        self._roles_accuracy(normalised_head_role_logits, role_tags)
        self._roles_f1_pro(normalised_head_role_logits, role_tags)
        self._roles_f1_opp(normalised_head_role_logits, role_tags)
        ...
        role_loss = normalised_head_role_logits[range_vector, child_index, role_tags]
        # We don't care about predictions for the symbolic ROOT token's head,
        # so we remove it from the loss.
        arc_loss = arc_loss[:, 1:]
        tag_loss = tag_loss[:, 1:]
        role_loss = role_loss[:, 1:]
        ...
        role_nll = -role_loss.sum() / valid_positions.float()
        return arc_nll, tag_nll, role_nll
    ...
    def _greedy_decode(
        ...
        head_role_logits = self._get_head_roles(
            head_role_representation, child_role_representation, heads
        )
        _, head_roles = head_role_logits.max(dim=2)

        return heads, head_tags, head_roles
    ...
    def _mst_decode(
        ...
        batch_size, sequence_length, role_representation_dim = head_role_representation.size()
        expanded_shape = [batch_size, sequence_length, sequence_length, role_representation_dim]
        head_role_representation = head_role_representation.unsqueeze(2)
        head_role_representation = head_role_representation.expand(*expanded_shape).contiguous()
        child_role_representation = child_role_representation.unsqueeze(1)
        child_role_representation = child_role_representation.expand(*expanded_shape).contiguous()
        # Shape (batch_size, sequence_length, sequence_length, num_head_tags)
        pairwise_role_logits = self.role_bilinear(head_role_representation, child_role_representation)
        # restrain the root role: always pro
        pairwise_role_logits[:,:,0] = 0.
        pairwise_role_logits[:,:,0,self._pro_idx] = 1.  # 100% central claim being pro
        ...
        normalized_pairwise_role_logits = F.log_softmax(pairwise_role_logits, dim=3).permute(
            0, 3, 1, 2
        )
        ...
        batch_energy_roles = torch.exp(normalized_arc_logits.unsqueeze(1) + normalized_pairwise_role_logits)
        return self._run_mst_decoding(batch_energy_functions, batch_energy_roles, lengths)

    @staticmethod
    def _run_mst_decoding(
            batch_energy_functions: torch.Tensor,
            batch_energy_roles: torch.Tensor,
            lengths: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        heads = []
        head_tags = []
        head_roles = []
        for energy_func, energy_role, length in zip(batch_energy_functions.detach().cpu(),
                                                    batch_energy_roles.detach().cpu(),
                                                    lengths):
            ...
            for child, parent in enumerate(instance_heads):
                ...
                instance_role_tags.append(role_ids[parent, child].item())
            ...
            instance_role_tags[0] = 0

            heads.append(instance_heads)
            head_tags.append(instance_head_tags)
            head_roles.append(instance_role_tags)
        ...
        return (
            ...
            torch.from_numpy(numpy.stack(head_roles)).to(batch_energy_functions.device),
        )
    ...
    def _get_head_tags(
        ...
        # # restrain the root function: always cc
        # new_p = torch.tensor([0.] * head_tag_logits.shape[2]).to(head_tag_logits.device)
        # new_p[self._cc_idx] = 1.
        # head_tag_logits[head_indices==0] = new_p
    ...
    def _get_head_roles(
        ...
        # # restrain the root role: always pro
        # new_p = torch.tensor([0.] * head_role_logits.shape[2]).to(head_role_logits.device)
        # new_p[self._pro_idx] = 1.
        # head_role_logits[head_indices==0] = new_p
